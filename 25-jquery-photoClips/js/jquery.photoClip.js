 ...
 *
 * @option_param {string} flipBtn 缩小图片按钮的选择器或者DOM对象
 */
...
(function(root, factory) {
  ...
  $.fn.photoClip = function(option) {
    if (!window.FileReader) {
      alert(
        "您的浏览器不支持 HTML5 的 FileReader API， 因此无法初始化图片裁剪插件，请更换最新的浏览器！"
      );
      return;
    }
  ...
  function photoClip(container, option) {
    var size = option.size,
      ...
      flipHBtn = option.flipHBtn,
      flipVBtn = option.flipVBtn,
    ...
    function resetScroll() {
      ...
      var posX = (clipWidth - imgWidth * myScroll.options.zoomStart) * 0.5,
        posY = (clipHeight - imgHeight * myScroll.options.zoomStart) * 0.5;
    ...
    function initEvent() {
      ...
      if (is_mobile) {
        ...
        $(flipVBtn).on("click", function(e) {
          ...
          hisScale.scaleX = 1;
          hisScale.scaleY = hisScale.scaleY === -1 ? 1 : -1;
        });
    ...
    function initClip() {
      ...
      canvas.style = "border: 1px solid #fff123";
    }
    ...
    //     ctx.scale(
    //       (outputWidth / clipWidth) * scale,
    //       (outputHeight / clipHeight) * scale
    //     );
    //   }
    ...
    function clipImg() {
      ...
      var dataURL = canvas.toDataURL(outputType, 1);
      $view.css("background-image", "url(" + dataURL + ")");
      $view.css("border", "solid 1px black");
      clipFinish.call($img[0], dataURL);
    }
    ...
    function loaclToLoacl($layerOne, $layerTwo, x, y) {
      // 计算$layerTwo上的x、y坐标在$layerOne上的坐标
      x = x || 0;
      y = y || 0;
    ...
    function calculateScale(width, height) {
      ...
      myScroll.options.zoomStart = Math.min(
        myScroll.options.zoomMax,
        getScale(containerWidth, containerHeight, width, height)
      );
    }
    ...
    function init() {
      ...
      // 创建裁剪视图层
      $clipView = $("<div class='photo-clip-view'>")
        .css({
          position: "absolute",
          left: "50%",
          top: "50%",
      ...
      $rotateLayer = $("<div class='photo-clip-rotateLayer'>").appendTo(
        $moveLayer
      );

      // 创建遮罩
      var $mask = $("<div class='photo-clip-mask'>")
        .css({
          position: "absolute",
          left: 0,
          top: 0,
        ...
        .appendTo($container);
      var $mask_left = $("<div class='photo-clip-mask-left'>")
        .css({
          position: "absolute",
          left: 0,
          right: "50%",
        ...
        .appendTo($mask);
      var $mask_right = $("<div class='photo-clip-mask-right'>")
        .css({
          position: "absolute",
          left: "50%",
          right: 0,
        ...
        .appendTo($mask);
      var $mask_top = $("<div class='photo-clip-mask-top'>")
        .css({
          position: "absolute",
          left: 0,
          right: 0,
        ...
        .appendTo($mask);
      var $mask_bottom = $("<div class='photo-clip-mask-bottom'>")
        .css({
          position: "absolute",
          left: 0,
          right: 0,
        ...
        .appendTo($mask);
      // 创建截取区域
      var $clip_area = $("<div class='photo-clip-area'>")
        .css({
