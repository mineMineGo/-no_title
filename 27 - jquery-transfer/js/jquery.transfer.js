...
(function($) {
  ...
  $.fn.transfer = function(options) {
    ...
    return this.each(function() {
      ...
      for (var i = 0; i < transfer.sourceItemList.length; i++) {
        sourceHtml += settings.itemTpl
          .replace(/\{\{value\}\}/g, transfer.sourceItemList[i].value)
          .replace(/\{\{text\}\}/g, transfer.sourceItemList[i].text);
      }
      for (var i = 0; i < transfer.targetItemList.length; i++) {
        targetHtml += settings.itemTpl
          .replace(/\{\{value\}\}/g, transfer.targetItemList[i].value)
          .replace(/\{\{text\}\}/g, transfer.targetItemList[i].text);
      }
      $html
        .find(".transfer-left-panel .transfer-panel-list")
        .append(sourceHtml);
      $html
        .find(".transfer-right-panel .transfer-panel-list")
        .append(targetHtml);
      ...
      _this.$sourceSelect = _this.find(
        ".transfer-left-panel .transfer-panel-list"
      );
      _this.$targetSelect = _this.find(
        ".transfer-right-panel .transfer-panel-list"
      );
      _this.$buttons = _this.find(".transfer-buttons");
      _this.$addBtn = _this.find(".btn-add-item");
      _this.$removeBtn = _this.find(".btn-del-item");
      _this.$checkboxInput = _this.find(
        ".transfer-panel-body .checkbox-input :checkbox"
      );
      _this.$checkAllInput = _this.find(
        ".transfer-panel-footer .checkbox-input :checkbox"
      );
      ...
      function checkBtnStatus(parent) {
        ...
        var checkboxLength = parent.find(
          ".transfer-panel-body .checkbox-input :checkbox"
        ).length;
        var checkedLength = parent.find(
          ".transfer-panel-body .transfer-panel-list :checkbox:checked"
        ).length;
        if (checkedLength > 0) {
      ...
      _this.$filterInput.keyup(function() {
        var value = $(this).val();
        var $itemList = $(this)
          .parents(".transfer-panel")
          .find(".transfer-panel-list");
        $itemList.find(".transfer-panel-item").show();
        if (value == "") {
        } else {
          //   $itemList.find(".transfer-panel-item").hide();
          //   $itemList
          //     .find(".transfer-panel-item[data-text*='" + value + "']")
          //     .show();
          $itemList
            .find(".transfer-panel-item")
            .not("[data-text*='" + value + "']")
            .hide();
        }
      });
      ...
      _this.$checkAllInput.click(function() {
        ...
        var $checkboxs = $parent.find(
          ".transfer-panel-list .transfer-panel-item :checkbox"
        );
      ...
      _this.$addBtn.click(function() {
        var $checkedBoxs = _this.$sourcePanel.find(
          ".transfer-panel-list .transfer-panel-item :checkbox:checked"
        );
        var html = "";
      ...
      _this.$removeBtn.click(function() {
        var $checkedBoxs = _this.$targetPanel.find(
          ".transfer-panel-list .transfer-panel-item :checkbox:checked"
        );
        var html = "";
        $checkedBoxs.each(function() {
  ...
  $.fn.transfer.defaults = {
    ...
    itemTpl:
      '<label class="transfer-panel-item" data-text="{{text}}" data-value="{{value}}">\
                       <span class="checkbox-input pull-left">\
